const detectDevTools = function() {
	...
	if (widthThreshold || heightThreshold) {
		console.log('%c⚠️ Hey! Tu fouilles dans le code? 👀', 'font-size: 24px; font-weight: bold; color: #ff0000;');
		console.log('%cJe te vois Kayou !', 'font-size: 18px; color: #4EC0CA;');
...
var sky, land, bird, pipe, pipeUp, pipeDown, scoreBoard, scoreBoard2, ready, splash;
...
var loadImages = function(){
	...
	scoreBoard2 = new Image();
	scoreBoard2.src = 'images/scoreboard2.png';
	scoreBoard2.onload = onImgLoad;
...
var deathAnimation = function(){
	if(splash){
		ctx.drawImage(splash, width / 2 - 250, height / 2 - 400);
		splash = undefined;
	}
	else {
        const randomBoard = Math.random() < 0.5 ? scoreBoard : scoreBoard2;
        ctx.drawImage(randomBoard, width / 2 - 500, height / 2 - 200);
        playend = true;
        playdata = [mode, score];
...
var drawSky = function(){
    ...
    var scale = Math.max(width / sky.width, height / sky.height);
    var newWidth = sky.width * scale;
    var newHeight = sky.height * scale;
    ...
    ctx.drawImage(sky, x, y, newWidth, newHeight);
}
...
var drawBird = function(){
    ctx.drawImage(
        bird,           // image source
        0,              // source x
        birdN * 48,     // source y (48 = 192/4 for each frame)
        68,             // source width (new sprite 68px)
        48,             // source height (192/4 = 48px per frame)
        birdPos,        // destination x
        birdY,          // destination y
        68,             // destination width
